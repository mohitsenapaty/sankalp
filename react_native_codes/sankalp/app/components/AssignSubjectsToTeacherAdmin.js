...
const allSec = ['A', 'B', 'C'];
//type Props = {};
export default class AssignSubjectsToTeacher extends React.Component{
    ...
    try{
      ...
      fetch(globalAssets.IP_IN_USE+'/fetchSubjectsAssignedToTeacher/'+this.state.user_token+'/', {
        ...
        body: JSON.stringify({
          ...
          teacher_name: this.props.navigation.state.params.i.fullname,
        }),
      })
    ...
    return this.state.assignedSubjectDataList.map((row_set, i)=>{
      return (
        <View key={i} style={{
          ...
          <Text>{row_set.class} </Text>
          <Text>{row_set.section}</Text>
          <Text onPress={()=>{this.deleteSubjectAssignmentAlert(row_set.subject_name, row_set.subject_id, row_set.class, row_set.section)}}>Delete Assignment</Text>
        </View>
      );
    });
  ...
  deleteSubjectAssignmentAlert = (subject_name, subject_id, class_, section) =>{
    Alert.alert(
      'Confirm Delete Teacher',
      'Do you want to delete the subject ' + subject_name + ' assigned to ' + this.props.navigation.state.params.i.fullname +'?',
      [
        {text: 'No', onPress: () => console.log('Cancel Pressed'), style: 'cancel'},
        {text: 'Yes', onPress: () => this.deleteSubjectAssignment(subject_id, class_, section)},
      ],
      { cancelable: false }
    );
  }
  deleteSubjectAssignment = (subject_id, class_, section) =>{
    //alert("Will delete " + i);
    try{
      //alert("a"); 
  ...
  confirmAssignmentAlert = () =>{
    Alert.alert(
      ...
      'Do you want to assign the subject ' + this.state.selectedSubject + 'of class '+this.state.selectedClass+this.state.selectedSec+' to the teacher '+this.props.navigation.state.params.i.fullname + '?',
      [
        {text: 'No', onPress: () => console.log('Cancel Pressed'), style: 'cancel'},
  ...
  confirmAssignemt = (i,j) => {
    ...
    try{
      ...
      .then((res) => {
        ...
        //alert("a");
        if (res.success === 1){
          alert("Subject assigned to Teacher.")
