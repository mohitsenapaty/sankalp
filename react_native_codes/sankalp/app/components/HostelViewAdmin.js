...
//type Props = {};
export default class HostelViewAdmin extends React.Component{
  ...
  displayHostelByRow(){
    return this.state.hostel_data.map((row_set, i)=>{
      if (i >= this.state.noticeCurrentPageNum * this.state.noticePageLength && i < (this.state.noticeCurrentPageNum+1)*this.state.noticePageLength)
        return (
          <View key={i} style={{
            ...
            <Text>Hostel Name      :  {row_set.hostel_name}</Text>
            <Text>Hostel Address   :  {row_set.hostel_address}</Text>
            <Text></Text>
            <Text style={stylesAdmin.DeleteLinkText} onPress={()=>{this.deleteHostelAlert(row_set)}}>Delete Hostel.</Text>
            <Text></Text>
          </View>
        );
    });
  ...
  displayhostels(){
    ...
    if (this.state.hostel_data.length == 0){
      return(
      ...
      return(
        <View style={stylesAdmin.InputContainer}>
          <Text>Currently these hostels have been added yet.</Text>
      ...
      <DrawerLayout
        ...
        <ScrollView style={stylesAdmin.Container}>
          <View style={stylesAdmin.InputContainer}>
            ...
            <TextInput onChangeText={(hostelName)=>this.setState({hostelName})} value={this.state.hostelName} style={stylesAdmin.Input} placeholder='Hostel Name'></TextInput>
            <Text>Hostel Address</Text>
            <TextInput onChangeText={(hostelAddress)=>this.setState({hostelAddress})} value={this.state.hostelAddress} style={stylesAdmin.Input} placeholder='Hostel Address'></TextInput>
            <TouchableOpacity onPress={() => this.addHostelAlert()} style={stylesAdmin.ButtonContainer}>
              <Text style={stylesAdmin.ButtonText}>ADD hostel</Text>
            </TouchableOpacity>
          </View>
          {this.displayhostels()}
          <Text></Text>
  ...
  addHostel = () =>{
    ...
    try{
      //alert("a"); 
      fetch(globalAssets.IP_IN_USE+'/addHostel/'+ this.state.user_token+'/'+ this.state.schoolName + '/', {
        method: 'POST',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
        ...
        body: JSON.stringify({
          ...
          hostel_address: this.state.hostelAddress,
        }),
      })
      .then((response) => response.json())
      .then((res) => {
        ...
        else{alert("Error adding hostel.");}
      })
      .catch((err)=>{
        alert("Network error. Please try again.");
      })
  ...
  deleteHostel = (i) =>{
    try{
      //alert("a"); 
      fetch(globalAssets.IP_IN_USE+'/deleteHostel/'+ this.state.user_token+'/'+ this.state.schoolName + '/', {
        method: 'POST',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
      ...
      .then((res) => {
        ...
        else{alert("Error deleting Hostel.");}
      })
      .catch((err)=>{
        alert("Network error. Please try again.");
      })
