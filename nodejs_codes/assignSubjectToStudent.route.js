...
router.post('/:pwd/', function(req, resp, next){
  ...
  if (loginType == 'Admin'){
    ...
    db_client.connect(function(err_){
      ...
      db_client.query("insert into student_subject_detail(student_id, subject_id) select a.student_id, b.subject_id from student_login as a, subject_details as b where a.fullname=$1 and b.subject_name=$2;",
        [student_name, subject_name], function(err, res)
      {
        if (err){
          console.log(err); 
          resp.send(login_data);
