...
router.post('/:pwd/:schoolName/', function(req, resp, next){
  ...
  if (loginType == 'Admin'){
    ...
    db_client.connect(function(err_){
      ...
      db_client.query("select distinct a.*, b.* from notification_detail a join notification_target b on a.notification_id=b.notification_id order by a.created_at desc;"
        , function(err, res)
      {
        if (err){
          console.log(err); 
    ...
    db_client.connect(function(err_){
      ...
      db_client.query("select * from notification_target a join notification_detail b on a.notification_id=b.notification_id where target_type='All' or target_type='All Teacher' or (target_type='Teacher' and target_id=$1) order by b.created_at desc;"
        ,[userID]
        , function(err, res)
      {
        if (err){
