...
router.post('/:pwd/:schoolName/', function(req, resp, next){
  ...
  if (loginType == 'Admin'){
    ...
    db_client.connect(function(err_){
      ...
      db_client.query("DELETE FROM student_academic_enrollment_detail WHERE student_id in (select student_id from student_login WHERE fullname=$1) ;"
        ,[fullname], function(err, res)
      {
        if (err){
          console.log(err); 
          resp.send(login_data);
        ...
        { //console.log(res);
          ...
          db_client.query("DELETE FROM student_login WHERE fullname=$1;",
            [fullname], function(err1, res1)
          {
            if (err1){
