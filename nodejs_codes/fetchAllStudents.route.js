...
router.post('/:pwd/:schoolName/', function(req, resp, next){
  ...
  if (loginType == 'Admin'){
    ...
    db_client.connect(function(err_){
      ...
      db_client.query("select a.*,b.class,b.section,b.roll_number from student_login a join student_academic_enrollment_detail b on a.student_id = b.student_id order by b.class, b.section, b.roll_number;", function(err, res)
      {
        if (err){
          console.log(err); 
          resp.send(login_data);
