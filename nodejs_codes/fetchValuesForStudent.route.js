var express = require('express');
var SHA224 = require('sha224');
var router = express.Router();
var pg = require('pg');
var conString1 = "postgres://postgres:postgres@localhost:5432/sankalp_";
var crypto = require('crypto');

router.post('/:pwd/:schoolName/', function(req, resp, next){

	console.log(req.body);
  //console.log(next);
  var schoolName = (req.params.schoolName).toLowerCase();
  //console.log(schoolName);
  var conString = conString1 + schoolName;
  //console.log(conString);

  var userid = req.body.user_id;
  var userID = parseInt(userid, 10);
  var loginType = req.body.loginType;
  var student_id = req.body.student_id;
  var exam_group_id = req.body.exam_group_id;
  
  var login_data = {'success':0,'data':{},'token':''};
  //console.log(SHA224(password, "utf8").toString('hex'));
  //enc_pwd = SHA224(password, "utf8").toString('hex');
  try{
    var api_key = crypto.createDecipher('aes-128-cbc', schoolName);
    var got_id = api_key.update(req.params.pwd, 'hex', 'utf8');
    got_id += api_key.final('utf8');
    console.log(got_id + " qqq" );
  }
  catch(error){
    console.log(error);
    resp.send(login_data);
    return;
  }
  if (got_id != userid){
    console.log(got_id);
    console.log(userid);
    resp.send(login_data);
    console.log("01");
    return;
  }

  if (loginType == 'Admin'){
    var db_client = new pg.Client(conString);
    db_client.connect(function(err_){

      if (err_){
        console.log(err); resp.send(login_data);
      }

      db_client.query("select a.fullname, b.roll_number, c.*, d.*, e.*, f.* from student_login a join student_academic_enrollment_detail b on a.student_id=b.student_id join student_csa c on a.student_id=c.student_id join student_ahs d on a.student_id=d.student_id join student_personal_trait e on e.student_id=a.student_id join student_remarks f on f.student_id=a.student_id where a.student_id=$1 and c.exam_group_id=$2 and d.exam_group_id=$2 and e.exam_group_id=$2 and f.exam_group_id=$2;"
        ,[student_id, exam_group_id]
        , function(err, res)
      {
        if (err){
          console.log(err); 
          resp.send(login_data);
          db_client.end(function(err1){

            if (err1){console.log(err1);}
          });
        }
        else
        { //console.log(res);
          console.log(res.rows.length);

          //var data_arr = {};
          //var data_dict = {'data_arr':data_arr}
          console.log(res.rows[0]);
          login_data['data'] = res.rows[0];
          login_data['success'] = 1;
          //login_data['token'] = got_id;

          resp.send(login_data);
          //close connection
          db_client.end(function(err1){

            if (err1){console.log(err1);}
          });
        }
      });
    });
  }
  else if (loginType == 'Teacher'){
    
  }
  else if (loginType == 'Student'){
    var db_client = new pg.Client(conString);
    db_client.connect(function(err_){

      if (err_){
        console.log(err); resp.send(login_data);
      }

      db_client.query("select a.fullname, b.roll_number, c.*, d.*, e.*, f.* from student_login a join student_academic_enrollment_detail b on a.student_id=b.student_id join student_csa c on a.student_id=c.student_id join student_ahs d on a.student_id=d.student_id join student_personal_trait e on e.student_id=a.student_id join student_remarks f on f.student_id=a.student_id where a.student_id=$1 and c.exam_group_id=$2 and d.exam_group_id=$2 and e.exam_group_id=$2 and f.exam_group_id=$2;"
        ,[userID, exam_group_id]
        , function(err, res)
      {
        if (err){
          console.log(err); 
          resp.send(login_data);
          db_client.end(function(err1){

            if (err1){console.log(err1);}
          });
        }
        else
        { //console.log(res);
          console.log(res.rows.length);

          //var data_arr = {};
          //var data_dict = {'data_arr':data_arr}
          console.log(res.rows[0]);
          login_data['data'] = res.rows[0];
          login_data['success'] = 1;
          //login_data['token'] = got_id;

          resp.send(login_data);
          //close connection
          db_client.end(function(err1){

            if (err1){console.log(err1);}
          });
        }
      });
    });
  }
  else{
    resp.send(login_data);
  }

});

module.exports = router;