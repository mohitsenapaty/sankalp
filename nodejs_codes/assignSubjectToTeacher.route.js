...
router.post('/:pwd/', function(req, resp, next){
  ...
  if (loginType == 'Admin'){
    ...
    db_client.connect(function(err_){
      ...
      db_client.query("insert into teacher_subject_detail(subject_id, teacher_id, class, section) select a.subject_id, b.teacher_id, $3, $4 from subject_details as a, teacher_login as b where a.subject_name=$2 and b.fullname=$1;",
        [teacher_name, subject_name, class_, section], function(err, res)
      {
        if (err){
          console.log(err); 
          resp.send(login_data);
