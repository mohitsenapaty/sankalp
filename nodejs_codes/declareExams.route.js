...
router.post('/:pwd/:schoolName/', function(req, resp, next){
  ...
  //console.log(next);
  schoolName = (req.params.schoolName).toLowerCase();
  ...
  if (loginType == 'Admin'){
    ...
    var db_client1 = new pg.Client(conString);
    db_client1.connect(function(err_){

      if (err_){
        console.log(err_); resp.send(login_data);
      }

      db_client1.query("select distinct a.student_id, b.term_final from exam_group_scoring a join exam_group_detail b on a.exam_group_id=b.exam_group_id where a.exam_group_id=$1;"
        ,[exam_group_id]
        , function(err, res)
      {
        if (err){
        ...
        { //console.log(res);
          ...
          generate_all_pdf(res.rows ,exam_group_id, conString);
...
async function generate_all_pdf(res_rows, exam_group_id, conString){

  //console.log(student_id + " " + exam_group_id + " " + conString);
  for (var i = 0; i < res_rows.length;i++){
    //student_id_list.push(res.rows[i].student_id);
    if (res_rows[i].term_final == 'N'){
      var result = await generate_pdf(res_rows[i].student_id, exam_group_id, conString);
      console.log(result + " " + res_rows[i]);
    }
    else{
      var result = await generate_pdf_full(res_rows[i].student_id, exam_group_id, conString);
      console.log(result + " " + res_rows[i]);
    }
  }
...
async function generate_pdf(student_id, exam_group_id, conString){
...
async function generate_pdf_full(student_id, exam_group_id, conString){
