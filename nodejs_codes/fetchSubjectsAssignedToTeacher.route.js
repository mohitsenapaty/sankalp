...
router.post('/:pwd/', function(req, resp, next){
  ...
  if (loginType == 'Admin'){
    ...
    db_client.connect(function(err_){
      ...
      db_client.query("SELECT a.*, b.subject_name FROM teacher_subject_detail a JOIN subject_details b on a.subject_id=b.subject_id where a.teacher_id in (SELECT teacher_id from teacher_login where fullname=$1);"
        ,[teacher_name]
        , function(err, res)
      {
        if (err){
