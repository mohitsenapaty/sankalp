...
router.post('/:pwd/', function(req, resp, next){
  ...
  if (loginType == 'Admin'){
    ...
    db_client.connect(function(err_){
      ...
      db_client.query("SELECT DISTINCT a.*, b.subject_name FROM teacher_subject_detail a JOIN subject_details b on a.subject_id=b.subject_id join exam_group_scoring c on b.subject_id = c.subject_id where a.teacher_id in (SELECT teacher_id from teacher_login where fullname=$1);"
        ,[teacher_name]
        , function(err, res)
      {
        if (err){
    ...
    db_client.connect(function(err_){
      ...
      db_client.query("SELECT DISTINCT a.*, b.* FROM teacher_subject_detail a JOIN subject_details b on a.subject_id=b.subject_id JOIN exam_group_scoring c on a.subject_id=c.subject_id where a.teacher_id=$1;"
        ,[userID]
        , function(err, res)
      {
        if (err){
